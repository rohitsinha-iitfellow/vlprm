...
class BatchProcessor:
    def __init__(self, verification_batches_dir: str = "verification_batches", 
        ...
        endpoint = azure_endpoint or "https://<your-endpoint>.openai.azure.com/"
        ...
        self.logger.info(f"API TIMEOUTS: connect={timeout.connect}s, read={timeout.read}s, write={timeout.write}s")
        ...
        try:
            ...
            if file_id:
                ...
                if error_sample_ids:
                    ...
                    print(f"  {len(error_sample_ids)} samples had errors")
                else:
            ...
            if status == "completed":
                ...
                if batch_response.errors:
                    for error in batch_response.errors.data:
                        ...
                        print(f" Batch error: {error.code} - {error.message}")
                ...
                print(f" Batch canceled: {filename}")
                return True
            ...
            while file_index < total_files or current_job:
                ...
                if current_job:
                    ...
                    try:
                        ...
                        print(f" Error in batch monitoring: {str(e)}")
    ...
    def cancel_my_active_batches(self) -> None:
        ...
        if not self.my_active_jobs:
            ...
            print("No active batch jobs to cancel for this processor")
            return
        ...
        for job in self.my_active_jobs[:]:  # Create a copy to avoid modification during iteration
            if job.batch_id:
                try:
                    ...
                    print(f"  Failed to cancel batch {job.batch_id}: {str(e)}")
            else:
        ...
        if cancelled_count > 0:
            ...
            print("Note: Only cancelled batches submitted by this processor instance")
...
def main(check_interval_minutes: int = 1, start_index: int = None, end_index: int = None, split: str = None, azure_endpoint: str = None, model: str = None):
    ...
    endpoint = azure_endpoint or "https://<your-endpoint>.cognitiveservices.azure.com/"
    
    print(f" Starting batch processing for split: {split} | model: {model}")
    print(f"Using Azure endpoint: {endpoint}")
    
    processor = BatchProcessor(
        verification_batches_dir=f"<base_path>/{model}/{split}/verification_batches",
        max_retries=10,
        azure_endpoint=endpoint,
